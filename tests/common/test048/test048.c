...
// user mode functions
// The assembly functions are already tagged with the .text.vumode section
// attribute.
...
uint8_t c_check_passed_in_arguments(uint8_t a0, uint8_t a1, uint8_t a2,
                                    uint8_t a3, uint8_t a4, uint8_t a5,
                                    uint8_t a6)
    __attribute__((section(".text.vumode"))) __attribute__((const));

uint8_t vsmode_function(void) __attribute__((section(".text.vsmode")))
__attribute__((const));

uint8_t get_bytes_to_copy(void);
int copy_bytes(void);
int compare_copied_bytes(void) __attribute__((section(".text.vumode")))
__attribute__((pure));
...
uint8_t vsmode_function(void) {
  ...
  if (run_function_in_vumode((uint64_t)asm_check_passed_in_arguments, 1, 2, 3,
                             4, 5, 6, 7) != DIAG_PASSED) {
    return DIAG_FAILED;
  }

  if (run_function_in_vumode((uint64_t)c_check_passed_in_arguments, 1, 2, 3, 4,
                             5, 6, 7) != DIAG_PASSED) {
    return DIAG_FAILED;
  }

  int bytes_to_copy = run_function_in_vumode((uint64_t)get_bytes_to_copy);
  ...
  for (uint8_t i = 0; i < 5; ++i) {
    // Read a Supervisor mode register to really make sure we're in
    // supervisor mode.
    ...
    if (run_function_in_vumode((uint64_t)copy_bytes) != 0) {
      return DIAG_FAILED;
    }
    ...
    if (run_function_in_vumode((uint64_t)compare_copied_bytes) != 0) {
      return DIAG_FAILED;
    }
...
int main(void) {
  ...
  uint8_t ret = run_function_in_vsmode((uint64_t)vsmode_function);
